...
class ChangePassPage extends ConsumerStatefulWidget {
  var mobile;
  ChangePassPage({super.key,required this.mobile});
              ...
              child: CommonFatButton(
                ...
                onPressed: () async {
                  if (_controllerPass.text == _controllerPassConfirm.text) {
                    var result = await ref.read(accountProvider).changePass(
                          context: context,
                          credential: widget.mobile.toString().trim(),
                          pass: Encryptor.encrypt(
                              "rana", _controllerPass.text.trim()),
                        );
                    ...
                    if (result == 'success') {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          backgroundColor: Colors.green,
                          content:
                              Text("Invalid Mobile No., User not Found!!")));
                    }
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        backgroundColor: Colors.green,
